...
Type objective_function<Type>::operator() ()
{
  ...
  DATA_SCALAR(omega_comp)   // assumed variance (?) for age comps
  ...
  for (int i=1;i<nyr;i++) Nij(i,0) = exp(logN(nage+i-1));
  Nij(nyr,0) = 0;
  ...
  for (int i=0;i<nyr;i++)
   for (int j=0;j<nage;j++)
    if (data_cm_comp(i,j) > 0)
     Like3 += data_cm_comp(i,j)*log(pred_cm_comp(i,j)/data_cm_comp(i,j));
