...
Type objective_function<Type>::operator() ()
{
 DATA_VECTOR(v1);
  ...
  vector<Type> v2_head = v2.head(2);  //  declare vector of real numbers, first 2 elements of v2
  REPORT(v2_head);                      //  v2.tail(2): last 2 elem.
  
  vector<Type> v3_segment = v3.segment(2,3); //segment of 3 elements,
  REPORT(v3_segment);                     // starting at 2nd element
  ...
  Type v1_times_v1_sum = (v1*v1).sum(); // inner product (dot product)
  ...
  matrix<Type>  m4 = m1*m2; // matrix multiplication
  REPORT(m4);
